...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim: tuple, size: int, batch_size: int = 32):
        ...
        self.obs_buf = np.zeros([size, 1, 105,80], dtype=np.float32)
        self.next_obs_buf = np.zeros([size, 1, 105,80], dtype=np.float32)
...
class Network(nn.Module):
    ...
    def __init__(
        ...
        self.LSTM = torch.nn.LSTM(128,  128, num_layers=1, bias=True, batch_first=True, dropout=0.0, bidirectional=False, proj_size=0, device=device)
        self.hidden = torch.zeros( 1, 128).to(device)
        self.cell = torch.zeros( 1, 128).to(device)
        self.out_dim = out_dim
        self.atom_size = atom_size
        self.support = support.to(device)
        self.lin1 = nn.Linear(2816, 128)
...
class DQNAgent:
    ...
    def train(self, num_frames: int, plotting_interval: int = 10000):
        ...
        for frame_idx in range(1, num_frames + 1):
            if(frame_idx % 100000 == 0):
              torch.save(self.dqn.state_dict(), "categorical_dqn_lstm.pth")
    ...
    def _plot(
        ...
        plt.plot(scores)
        scoresFig.savefig('scoresPlotLSTM.png')
        plt.close(scoresFig)
        lossesFig = plt.figure()
        plt.title('loss')
        plt.plot(losses)
        plt.xlabel('Step')
        lossesFig.savefig('LossPlotLSTM.png')
        plt.close(lossesFig)
        #plt.subplot(133)
        #plt.title('epsilons')
        #plt.plot(epsilons)
...
#torch.save(agent.dqn.state_dict(), "categorical_dqn_final_LSTM.pth")
#agent.dqn.load_state_dict(torch.load("categorical_dqn_final_LSTM.pth", weights_only=True))
